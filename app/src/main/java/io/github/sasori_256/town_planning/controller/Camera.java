...
public class Camera {
     ...
     * @param offsetX X座標のオフセット
     * @param offsetY Y座標のオフセット
     * @return アイソメトリック座標
     */
    public Point2D.Double screenToIso(int screenX, int screenY, int offsetX, int offsetY){
        double adjX = screenX - offsetX;
        double adjY = screenY - offsetY;
     ...
     * @return スクリーン座標
     */
    public Point isoToScreen(double isoX, double isoY, int offsetX, int offsetY){
        double screenX = (isoX - isoY) * (this.cellWidth / 2.0) + offsetX;
        double screenY = (isoX + isoY) * (this.cellHeight / 2.0) + offsetY;
